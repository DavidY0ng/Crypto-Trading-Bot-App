...
{:else}
<BackHeader
	path={$storeLocal.prevUrl == '/' ? '/' : '/bots'}
	layout="flex items-center bg-white pb-2"
>
	<div class="flex justify-start flex-grow font-semibold flex-2 h3">Martingale Trading</div>
</BackHeader>
<div class="flex flex-col flex-grow p-3 bg-white">
	<div class="flex flex-col flex-grow p-3">
		<button
			class="flex items-center gap-2 mb-3 font-semibold"
		...
		<div class="flex flex-col mb-5">
			<RadioGroup
				...
				rounded="rounded-md"
				border="border-none"
				class="p-[2px] flex text-gray-400 "
			>
					...
					<div class="flex items-start justify-between" on:click={() => onSelectCoin(coin)}>
						<div class="">
							<div class="flex items-end gap-1">
								...
								<div class="text-xs font-thin text-gray-400">/ USDT</div>
							</div>
							<div class="text-xs font-thin text-gray-400/80">Vol 1.91B</div>
						</div>

						<div class="flex flex-col">
							<div class="font-semibold leading-none">68,228.3</div>
							<!-- <div class="text-xs text-gray-400">
                            ≈ 68,228.3 USD
                        </div> -->
							<div class="text-xs text-right text-green-500">+1.19%</div>
						</div>
					</div>
				{/each}
			</div>
