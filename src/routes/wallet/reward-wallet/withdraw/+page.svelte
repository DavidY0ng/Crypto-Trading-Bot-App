...
<BackHeader path="/wallet/reward-wallet" layout="flex items-center bg-white pb-2">
	<div class="flex justify-center flex-1 h3">Withdraw</div>
...
<div class="flex flex-col p-3">
	<div class="flex flex-col gap-5 p-5 shadow-md card">
		<div class="flex flex-col gap-1">
			<div class="flex text-sm text-gray-400">Select Coin</div>
		...
		<div class="flex flex-col gap-1">
			<div class="flex text-sm text-gray-400">Address</div>
			<input
				class="p-2 px-3 overflow-hidden text-sm bg-white border rounded-md"
				value={$storeUserInfo.address}
				readonly
			/>
		</div>

		<div class="flex flex-col gap-1">
			<div class="flex text-sm text-gray-400">Network</div>
		...
		<div class="flex flex-col gap-1">
			<div class="flex text-sm text-gray-400">Amount</div>
			<div>
				<input
					type="number"
					pattern="[0-9]*"
					class="p-2 px-3 bg-white border rounded-md shadow-sm {confirmCheck == true
						? 'text-gray-400'
						: ''}"
					bind:value={amountValue}
					placeholder="Enter an amount"
					readonly={confirmCheck}
				/>
			</div>
		...
		<div class="flex flex-grow">
			<button
				on:click={() => submitWithdrawal($withdrawInfo.sn, amountValue)}
				class="flex justify-center flex-grow text-white rounded-md shadow-md btn bg-primary-500"
