<script lang='ts'>
	...
	import { showAuthenticatorModal } from "$lib/stores/store";
	...
	$: if (dialog && showAuthenticatorModal) {
		dialog.showModal();
	}
	...
	function closeModal() {
		...
		showAuthenticatorModal.set(false);
		dispatch('close')
	}
</script>
...
{#if $showAuthenticatorModal }
	<dialog class="{modalClass} w-[320px]"
		bind:this={dialog}
		on:close={() => showAuthenticatorModal.set(false)}
